...
RSpec.describe AiGiftSuggestionsController, type: :controller do
  ...
  describe "POST #toggle_wishlist" do
    ...
    it "when not saved: creates wishlists for all planners" do
      suggestion = make_suggestion(title: "NewSave")
      request.env["HTTP_REFERER"] = "/back"

      post :toggle_wishlist, params: { event_id: event.id, id: suggestion.id }

      planner_wls = Wishlist.where(
        ai_gift_suggestion_id: suggestion.id,
        user_id: [owner.id, co_planner.id]
      )
  ...
  describe "GET #library" do
    ...
    let(:collab_er) do
      EventRecipient.create!(user: collab_owner, event: collab_event, recipient: collab_recipient, budget_allocated: 10.0)
    end

    before do
      allow(Event).to receive(:accessible_to).with(owner)
                                             .and_return(Event.where(id: [event.id, collab_event.id]))
    ...
    it "scope=collab shows only collaborator events" do
    ...
    it "applies filters correctly" do
      ...
      expect(assigns(:suggestions).map(&:title)).to eq(["MineNew"])
    end
  end
